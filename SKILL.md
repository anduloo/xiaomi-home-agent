...
收到用户指令后，按以下顺序处理，避免误操作或阻塞：

1. **未登录 / 无凭证**：先检查 `config/auth.json` 是否存在；不存在则运行 `python scripts/generate_qr.py` 生成二维码，由用户用米家 APP 扫码授权，**不要**直接跑阻塞式 `mijiaAPI login` / `mijiaAPI -l`。`list_devices.py` / `control_device.py` 现在会在执行前**运行时强校验**：mijiaAPI 未装 / `auth.json` 缺失或为空 / 登录已过期，会直接打印明确错误并以非零退出（JSON 模式带 `need_auth` 标记），无需 agent 自行判断，也不会冒出未捕获 traceback。

   **凭证过期「调用前检测」**：`_auth_guard.require_auth()` 在构造 mijiaAPI 实例**之前**读取 `auth.json` 的 `expireTime`（毫秒时间戳），已过期则直接失败并给出可读过期时间、退出码 6，避免浪费一次 API 调用。
2. **查询类**（"查看设备" / "温度多少" / "有哪些灯"）→ `list_devices.py` / `get_device_status.py`。
3. **控制类**（"打开 / 关闭 / 调亮度"）→ `control_device.py`；用 `--name` 模糊匹配时若命中多个设备，**先向用户确认唯一目标**再执行，避免误控。
4. **场景类**（"执行 XX 场景"）→ `run_scene.py`，默认 dry-run，需 `--yes` 确认后才真正执行。
5. **耗材 / 规格**（"滤芯寿命" / "设备规格"）→ `list_consumables.py` / `get_device_info.py`。

> 跨平台说明：本技能脚本统一用 `python` 调用（Windows 的 WorkBuddy 托管 Python 即为 `python`；macOS / Linux 若未建别名可用 `python3`）。**不要写死绝对路径**，始终先进入技能目录再执行脚本。
...
```bash
# 跨平台定位本技能目录（不要写死绝对路径）：
#   Windows / WorkBuddy: C:\Users\<用户名>\.workbuddy\skills\xiaomi-home-agent
#   macOS / Linux:       ~/.workbuddy/skills/xiaomi-home-agent
# 进入该目录后安装依赖：
...
```bash
# 打开设备
python scripts/control_device.py --did <设备ID> --action turn_on

# 关闭设备
python scripts/control_device.py --did <设备ID> --action turn_off

# 使用设备名称
python scripts/control_device.py --name "床头灯" --action turn_on
```
...
```bash
# 打开/关闭设备
python scripts/control_device.py --did <did> --action turn_on
python scripts/control_device.py --did <did> --action turn_off

# 设置亮度 (0-100)
python scripts/control_device.py --did <did> --action set_brightness --value 50

# 设置温度
python scripts/control_device.py --did <did> --action set_temperature --value 26

# 使用设备名称（模糊匹配）
python scripts/control_device.py --name "吸顶灯" --action turn_off
```
...
```bash
# 场景1: 床头灯打开时，自动关闭吸顶灯
python scripts/auto_scene_bedside.py

# 场景2: 吸顶灯打开时，自动关闭床头灯
python scripts/auto_scene_ceiling.py
```

### 设备状态查询

```bash
# 推荐：用本技能脚本，已自动覆盖「标准 + BLE(1000+)」双范围，避免静默返回 -704040003
python scripts/get_device_status.py --did <设备ID>
```

如需手动读取属性（如青萍空气检测仪），**必须同时覆盖标准范围与 BLE 范围**，否则 BLE 传感器会静默失败：
...
api = mijiaAPI()
did = '<设备ID>'
# 标准范围 siid=2/3 的 1-9 + BLE 范围 1003/1005/1078/1079/1080 + 电池 1003
std = [{'did': did, 'siid': s, 'piid': p} for s in (2, 3) for p in range(1, 10)]
ble = [{'did': did, 'siid': 2, 'piid': p} for p in (1003, 1005, 1078, 1079, 1080)] + [{'did': did, 'siid': 4, 'piid': 1}]
result = api.get_devices_prop(std + ble)
for item in result:
    if item.get('code') == 0:
        print('siid=%s piid=%s: %s' % (item.get('siid'), item.get('piid'), item.get('value')))
"
```
...
**正确做法**：查询传感器时必须同时扫描 `siid=2, piid=1-9` + `siid=3, piid=1-7` + `siid=2, piid=(1005,1078,1079,1080)` + `siid=4, piid=(1,1003)`。直接用 `get_device_status.py` 即可自动覆盖上述全部范围。
...
│   ├── _auth_guard.py         # 登录态运行时强拦截（list/control 共用）
